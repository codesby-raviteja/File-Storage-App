...
directoryRouter.get("/directory/:id?",authMiddleware ,async (req, res) => {
  ...
  try {
    const id = req.params.id || req.userObj.rootDirId
...
directoryRouter.post("/directory/:parentdirid?",authMiddleware, async (req, res) => {
  try {
    const parentDirId = req.params.parentdirid || req.userObj.rootDirId
    const dirName = req.body.newDirectoryName || "New Folder"

    const id = crypto.randomUUID()
    const newDirectory = { id, dirName, parentDirId, files: [], folders: [] }
...
directoryRouter.delete("/directory/:dirid", authMiddleware ,async (req, res) => {
  try {
